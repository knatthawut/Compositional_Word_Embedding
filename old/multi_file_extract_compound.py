...
def main():
    ...
    for filename in filenames:
        ...
        process = Process(target=extract_compound.extract_compound(filename,out_filename),args=[filename,out_filename])
        # processes.append(process)
        process.start()

    #cat file
    # cmd = 'cat {}_tmp*.out > {}'.format(input_file,output_file)
    # return_value = subpirocess.call(cmd,shell=True)
