...
import function_files_specific_word
import json
	...
	for line in train_data[start:start+bs]:
		...
		if len(data) == 2:		
			if data[0] in model:
				...
				train.append(np.array(function_files_specific_word.getSample(data[1].split(),model), dtype = np.float).astype(np.float32))   
			else:
...
# ***************_specific_word
	...
	new_train_data=['DBPEDIA_ID/hot_dog	hot dog','DBPEDIA_ID/red_tape	red tape','DBPEDIA_ID/white_elephant	white elephant','DBPEDIA_ID/real_estate	real estate',
	...
	print('Loading Data ...')
	test, t_target = function_files_specific_word.loadData(train_data,wordvec,0,batch_size)
...
# ********_specific_word
	if __name__ == '__main__':
		...
		for epoch in range(int(training_iters/10)):
			for _ in range(10):
				try:
					train, label = function_files_specific_word.loadData(train_data,wordvec,(batch_size*count)%num_lines,batch_size)
					count = count + 1
...
# *******
	...
	if __name__ == '__main__':	   
		for t in test_data:
			test = function_files_specific_word.loadDataTest(t,wordvec)
			...
			rn = rnn_vec[0].tolist()
			avg = (avg/c).tolist()
			dicts.update({key:	{  'avg':avg,'rnn':rn_o,'rnn_att':rn  } 	})
