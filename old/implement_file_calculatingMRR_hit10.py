...
vectorFile = './'+type_model+'/wiki-db_more50_'+str(n_input)+'_w20'# sys.argv[1]
...
# ********
	if __name__ == '__main__':
		...
		prev_error = 0.0
		error_now = 0.0
		for epoch in range(int(training_iters/5000)):
			for _ in range(10):
				try:
...
# *******
	...
	if __name__ == '__main__':	   
		...
		for t in test_data:
			...
			wordss = t.split()
			words = wordss[0]
			vec_for_file = " ".join(str(x) for x in rnn_vec[0])
			file.write(dicR[words]+'\t'+vec_for_file + '\n')

			if np.linalg.norm(rnn_vec_o - ref_vec) > np.linalg.norm(rnn_vec - ref_vec):
			...
			if np.linalg.norm(avg_vec - ref_vec) > np.linalg.norm(rnn_vec - ref_vec):
				eucliden_acc_a = eucliden_acc_a + 1.0
			if (1-scipy.spatial.distance.cosine(rnn_vec, ref_vec)) > (1-scipy.spatial.distance.cosine(avg_vec, ref_vec)):
				cosine_acc_a = cosine_acc_a + 1.0

			if np.linalg.norm(avg_vec - ref_vec) > np.linalg.norm(rnn_vec_o - ref_vec):
				eucliden_acc_o = eucliden_acc_o + 1.0
			if (1-scipy.spatial.distance.cosine(rnn_vec_o, ref_vec)) > (1-scipy.spatial.distance.cosine(avg_vec, ref_vec)):
				cosine_acc_o = cosine_acc_o + 1.0

		file.close()

		print(eucliden_acc/counter,cosine_acc/counter)
		print('for each one:\n')
		print('attention ->',eucliden_acc_a/counter,cosine_acc_a/counter)
		print('original ->',eucliden_acc_o/counter,cosine_acc_o/counter)
...
# ***************
	...
	for t in MRR_test_data:
		...
		rank_avg, hit_avg = function_files.ranking(ref_vec,avg_vec,wordvec)
		rank, hit = function_files.ranking(ref_vec,rnn_vec,wordvec)
		rank_o, hit_o = function_files.ranking(ref_vec,rnn_vec_o,wordvec)
		
		summary_avg = summary_avg + rank_avg
		hit_10_avg = hit_10_avg + hit_10_avg

		summary = summary + rank
		hit_10 = hit_10 + hit

		summary_o = summary_o + rank_o
		hit_10_o = hit_10_o + hit_o
		...
		if (count % 100) == 0:
			...
			pp.pprint('-hit@10_attention Right Now:'+str(hit_10/int(count)))
			pp.pprint('---')
			pp.pprint('-MRR Right Now:'+str(summary_o/float(count)))
			pp.pprint('-hit@10 Right Now:'+str(hit_10_o/int(count)))
			pp.pprint('---')
			pp.pprint('-MRR_avg Right Now:'+str(summary_avg/float(count)))
			pp.pprint('-hit@10_avg Right Now:'+str(hit_10_avg/int(count)))
			pp.pprint('---------')
	...
	with open('MRR/p_value_'+str(n_input)+'_'+str(fold_num)+'_'+'mixlast_avg.json', 'w') as outfile:
		json.dump(p_value_avg, outfile)

	avg_MRR = avg_MRR + (summary/float(count-1))
	avg_hit = avg_hit + (hit_10/int(count-1))
	avg_MRR_o = avg_MRR_o + (summary_o/float(count-1))
	avg_hit_o = avg_hit_o + (hit_10_o/int(count-1))

	avg_MRR_avg = avg_MRR_avg + (summary_avg/float(count-1))
	avg_hit_avg = avg_hit_avg + (hit_10_avg/int(count-1))
...
print("\navg_last_epoch_error_with_attention:"+str(avg_last_epoch_error/1) )
print("avg_eucliden_acc_with_attention:"+str(avg_eucliden_acc/1) )
print("avg_cosine_acc_with_attention:"+str(avg_cosine_acc/1) )
print("avg_MRR_with_attention:"+str(avg_MRR/1) )
print("avg_hit@10_with_attention:"+str(avg_hit/1) )

print("----------\n")

print("avg_last_epoch_error_without_attention:"+str(avg_last_epoch_error_o/1) )
print("avg_eucliden_acc_without_attention:"+str(avg_eucliden_acc_o/1) )
print("avg_cosine_acc_without_attention:"+str(avg_cosine_acc_o/1) )
print("avg_MRR_without_attention:"+str(avg_MRR_o/1) )
print("avg_hit@10_without_attention:"+str(avg_hit_o/1) )


print("----------\n")

print("avg_MRR_without_attention:"+str(avg_MRR_avg/1) )
print("avg_hit@10_without_attention:"+str(avg_hit_avg/1) )
