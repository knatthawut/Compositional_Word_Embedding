'''
Output the numpy vector from compositional models for visualize
...
# Parse the arguments
parser = argparse.ArgumentParser(description='Use baseline to convert compounds in input file and save the numpy array to output')
...
parser.add_argument('--input', type=str, metavar='', required=True, help='Path to input file')
parser.add_argument('--output', type=str, metavar='', required=True, help='Path to output file')
...
def wordTovec(X_word,baseline,word_vec_dict,embedding_dim):
    '''
    Function:
        convert the compound (list of word_id) into vector by load the Word2Vec model
    Input:
...
def readData_word(input_file,word_vector):
    '''
    Function:
        Read input_file and convert to word_id
    Input:
        input_file(str): name of the input file
        word_vector: word_vector model to convert word into word_id
    Output:
        X_test_word: [Baltimore_Sun]
        X_test_word_id: [[1,2]]
    '''
...
def writeVectorFile(output_file, words, vectors):
    '''
    Function: 
        Write word and vector of that word into output_file
    Input:
        output_file (str): name of the output file
        words (list of str): list of compounds
        vectors (list of np_array): list of compound vector
    Output: None
...
def main():
    ...
    X_test_word , X_test_word_idx = readData_word(test_data_file,word_vector)
    print('Read the list of compound!')
